...
def handle_chat(human_msg: str):
    ...
    logging.info(f"Received human_msg: {human_msg}")

    # Check to make sure the JSON is valid
    try:
        json.loads(human_msg)
    except json.JSONDecodeError:
        logging.error(f"Invalid JSON received: {human_msg}")
    ...
    logging.info(f"Sent response: {response.text}")
...
async def handle_chat(human_msg: str):
    print("Received payload:", human_msg)
    """
    Endpoint to handle chat.
    Receives a message from the user, processes it, and returns a response from the model.
    """
